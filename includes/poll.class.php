...
class poll {
            ...
            public function getResult($pollID){
                $resultData = array();
                if(!empty($pollID)){
                    $sql = "SELECT p.subject, count(v.id) as total_votes FROM ".$this->voteTbl." as v LEFT JOIN ".$this->pollTbl." as p ON p.id = v.poll_id WHERE poll_id = ?";
                    $pollResult = $this->getQuery($sql, 'i', [$pollID], 'single');
                    if(!empty($pollResult)){
                        ...
                        $sql2 = "SELECT o.id, o.name, (
                                    SELECT COUNT(*)
                                    FROM " . $this->voteTbl . " v
                                    WHERE v.poll_id = ? AND v.poll_option_id = o.id
                                  ) as vote_count
                                  FROM " . $this->optTbl . " o
                                  WHERE o.poll_id = ?";
                        $optResult = $this->getQuery($sql2, 'ii', [$pollID, $pollID]);
                        if(!empty($optResult)){
