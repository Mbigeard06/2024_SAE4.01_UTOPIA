    ...
    if (empty($title) || empty($date) || empty($headline) || empty($description)) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($resultCheck > 0) {
                ...
                $sql = "INSERT INTO events (event_by, title, date_created, event_date, event_image, headline, description) "
                     . "VALUES (?, ?, NOW(), ?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    header("Location: ../create-event.php?error=sqlerror");
                    ...
                    mysqli_stmt_bind_param($stmt, "mssssss", $_SESSION['userId'], $title, $date, $FileNameNew, $headline, $description);
                    mysqli_stmt_execute($stmt);
